...
public class cmdAuto_AutoAlignAndShoot extends Command {
  ...
  public cmdAuto_AutoAlignAndShoot(subDrive drive, subHood hood, subShooter shooter, subBed bed, subFeeder feeder, subIntake intake) {
    this.drive = drive;
    this.hood = hood;
  ...
  public void execute() {
    ...
    {
      timer.start();
      drive.applyRequest(() -> kdrive.withVelocityX(0).withVelocityY(0).withRotationalRate(turnCommand())).execute();
      hood.setPosition();
